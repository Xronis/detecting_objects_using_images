...
def forward(images, num_classes, upsampling_factor=8):
    ...
    Args:
        images (tensorflow.placeholder): Network input placeholder [batches, height, width, channels]
        num_classes (int): Number of class for classification
        upsampling_factor (int, optional): Upsample factor

    Returns:
        class_head (Tensor): Model output for logits, with shape [batches, num_classes, height, width]
        bbox_head (Tensor): Model output for Bounding box, with shape [batches, 4, height, width]
        depth_head (Tensor): Model output for depth, with shape [batches, 1, height, width]
    """

    resnet_features = _get_resnet_features(images)

    class_head = _create_task_net(resnet_features, num_classes, upsampling_factor=upsampling_factor)
    class_head = tf.nn.softmax(class_head, dim=1)

    bbox_head = _create_task_net(resnet_features, 4, upsampling_factor=upsampling_factor)
    depth_head = _create_task_net(resnet_features, 1, upsampling_factor=upsampling_factor)
...
def _create_task_net(inputs, output_features, upsampling_factor):
    ...
    Args:
        inputs (Tensor): A Tensor of rank N+2 of shape [batch_size] + input_spatial_shape + [in_channels]
