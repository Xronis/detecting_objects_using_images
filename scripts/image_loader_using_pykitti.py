...
def _transfort_images_to_npy_files(basedir, date, drive):
    # The 'frames' argument is optional - default: None, which loads the whole dataset.
    # Calibration, timestamps, and IMU data are read automatically.
    # Camera and velodyne data are available via properties that create generators
    # when accessed, or through getter methods that provide random access.
    ...
    for image in data.cam2:
        np.save('file_{}'.format(np.random.randint(1, 1000000)), np.array(image))
...
def transform_images(date='2011_09_26', basedir='E:\Documents\KITTI\Raw'):
    folder = basedir+'\\{}'.format(date)
    drives = [name for name in os.listdir(folder) if os.path.isdir(folder+'\\{}'.format(name))]
    ...
    for drive in drives:
        drive = drive.split('_')[-2]
        _transfort_images_to_npy_files(basedir, date, drive)
